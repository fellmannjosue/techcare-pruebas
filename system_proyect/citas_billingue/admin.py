...
class AppointmentAdmin(AuditAdminMixin):
...
class GradeAdmin(AuditAdminMixin):
    list_display = ('name',)

# Configuración para el modelo Subject
@admin.register(Subject_bl)
class SubjectAdmin(AuditAdminMixin):
...
class RelationshipAdmin(AuditAdminMixin):
    list_display = ('name',)
